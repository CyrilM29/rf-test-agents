...
Préférence d'écriture de l'auteur, valable dans tous ses dépôts et posée dans
ses instructions globales : le caractère est refusé dans TOUTE la rédaction
(docs, README, ``specs/``, mémoire projet, docstrings et commentaires, chaînes
émises, définitions d'agents, workflows CI, config). Une règle en prose n'est
pas un contrat : ce garde la rend mécanique, dans l'esprit de
...
* les artefacts de run (``results/``, ``dist/``), régénérés et non rédigés.
...
# Répertoires hors périmètre : artefacts de run (sorties Robot, builds).
EXEMPT_PREFIXES = (
    "results/",
    "dist/",
)
