"""Décline les définitions d'agents (source canonique :
``.claude/agents/rf-*.md``, format subagent Claude Code) en chat modes GitHub
Copilot pour VS Code (``.github/chatmodes/<name>.chatmode.md``).

Porté depuis le projet SAPFX (même auteur), où le mécanisme est né. Le
principe : une source unique, des cibles GÉNÉRÉES (jamais éditées à la main),
et un mode ``--check`` qui vérifie que les cibles committées correspondent
encore à la source. C'est la réponse mécanique à la convention #7 de ce dépôt
(« les définitions de `.claude/agents/` sont la source canonique ; si d'autres
formats d'assistants en sont dérivés, les GÉNÉRER, jamais les forker »).

Le corps (les instructions de l'agent) est copié tel quel — seul le
frontmatter change de dialecte :
