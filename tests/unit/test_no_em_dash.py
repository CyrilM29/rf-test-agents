...
@pytest.mark.parametrize("rel, scanned", [
    ...
    ("scripts/check_conventions.py", True),
    ("results/output.xml", False),
    ("specs/mon-plan.md", True),
])
...
def test_check_ignore_les_artefacts_de_run(tmp_path):
    # results/ est régénéré à chaque run : le garde n'a rien à y corriger,
    # et l'y faire échouer rendrait le hook post-édition ininterprétable.
