...
5. **Write the suite** at its ventilated path — `tests/robot/api/<slug>.robot`,
   ...
   - **provenance marker** (the spec is the source of truth): after writing
     the suite, stamp it — `python scripts/check_spec_sync.py --stamp
     <suite path> specs/<slug>.md`. The marker embeds the spec's content hash;
     `check_spec_sync.py` then fails whenever the spec changes without a
...
`common.resource`), any `variables/` file created, the two gate results (dry
run / live run) with their real status, and anything you had to leave open.
